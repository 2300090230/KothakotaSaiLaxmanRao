...
export default function Skills({ openAddModal = false, onResetQuickAction }) {
  ...
  const handleEditSkill = async (e) => {
    ...
    try {
      ...
      // Use the original skill name for the API endpoint
      const originalSkillName = selectedSkill.skillname || selectedSkill.name;
      
      await axios.put(`${config.url}/skills/update/${encodeURIComponent(originalSkillName)}`, formData, {
        timeout: 10000,
        headers: {
          'Content-Type': 'application/json',
        }
  ...
  const handleDeleteSkill = async (skillName) => {
    if (!skillName) {
      setError('Skill name is missing. Cannot delete.');
      console.error('Delete attempt failed: skillName is undefined or empty');
      return;
    }
    ...
    if (window.confirm(`Are you sure you want to delete ${skillName}?`)) {
      try {
        ...
        console.log(`Sending DELETE request to: ${config.url}/skills/delete/${encodedSkillName}`);
        
        const response = await axios.delete(`${config.url}/skills/delete/${encodedSkillName}`, {
          timeout: 10000,
          headers: {
            'Content-Type': 'application/json',
          }
  ...
  const getSkillIcon = (skill) => {
    if (skill.iconUrl) {
      return (
        <img 
          src={skill.iconUrl} 
          alt={skill.skillname || skill.name} 
    ...
    return (
      <div className="w-8 h-8 bg-gradient-to-br from-sky-100 to-sky-200 rounded-md flex items-center justify-center">
        <span className="text-sky-700 font-semibold text-sm">
          {(skill.skillname || skill.name || '').charAt(0).toUpperCase()}
        </span>
      </div>
    );
  };
                  ...
                  <div key={skill.id} className="bg-white/80 backdrop-blur-sm border border-sky-200/50 rounded-xl p-4 sm:p-6 hover:shadow-lg hover:shadow-sky-100/50 transition-all duration-300 transform hover:scale-105">
                    <div className="flex items-start justify-between mb-3 sm:mb-4">
                      ...
                      <div className="flex items-center space-x-1 sm:space-x-2 flex-shrink-0 ml-2">
                        ...
                        <button
                          onClick={() => handleDeleteSkill(skill.skillname || skill.name)}
              ...
              <div className="bg-white/80 backdrop-blur-sm px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-base sm:text-lg font-medium bg-gradient-to-r from-black to-sky-700 bg-clip-text text-transparent pr-2 truncate">
                    Edit: {selectedSkill?.skillname || selectedSkill?.name}
                  </h3>
                  <button
