  ...
  const getSkillIcon = (skill) => {
    if (skill.iconUrl) {
      return (
        <div className="relative group">
          <img 
            src={skill.iconUrl} 
            alt={skill.name || skill.skillname} 
            className="w-10 h-10 mx-auto mb-2 transition-transform duration-300 group-hover:scale-110"
            onError={(e) => {
              e.target.onerror = null;
              e.target.style.display = 'none';
              e.target.parentNode.innerHTML = `<div class="w-10 h-10 rounded-xl bg-gradient-to-br from-sky-100 to-sky-200 text-sky-600 flex items-center justify-center font-bold text-lg shadow-md">${(skill.name?.charAt(0) || skill.skillname?.charAt(0) || '?').toUpperCase()}</div>`;
            }}
          />
        </div>
      );
    ...
    return (
      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-sky-100 to-sky-200 text-sky-600 flex items-center justify-center font-bold text-lg mx-auto mb-2 shadow-md">
        {(skill.name?.charAt(0) || skill.skillname?.charAt(0) || '?').toUpperCase()}
      </div>
    );
  };
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                  {skills[activeCategory] && skills[activeCategory].length > 0 ? (
                    skills[activeCategory].map((skill, index) => (
                      <SkillCard key={skill.id || skill._id || `${activeCategory}-${index}`} skill={skill} index={index} />
                    ))
                  ) : (
